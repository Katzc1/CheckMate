...
public class ChessBoard extends JFrame {
   ...
   public static int[] getNumPieces() {
   	...
   	for(int r = 0; r < 8; r++) {
   		for(int f = 0; f<8; f++) {
   			if(ChessBoard.getSquare(r, f).getOccupancy()) {
   				if(ChessBoard.getSquare(r, f).getOccupant().getColor().equalsIgnoreCase("White")) {
       				output[0]++;
       			} else {
       				output[1]++;
       			}
   ...
   public static Piece[] getWhitePieces() {
   	int counter = getNumPieces()[0];
   	Piece[] output = new Piece[counter];
   	for(int r = 0; r < 8; r++) {
   		for(int f = 0; f<8; f++) {
   			if(ChessBoard.getSquare(r, f).getOccupancy()) {
   				if(ChessBoard.getSquare(r, f).getOccupant().getColor().equalsIgnoreCase("White")) {
       				if(counter == 0) {
       					...
       					counter--;
       					output[counter] = ChessBoard.getSquare(r, f).getOccupant();
   ...
   public static Piece[] getBlackPieces() {
   	int counter = getNumPieces()[1];
   	Piece[] output = new Piece[counter];
   	for(int r = 0; r < 8; r++) {
   		for(int f = 0; f<8; f++) {
   			if(ChessBoard.getSquare(r, f).getOccupancy()) {
   				if(ChessBoard.getSquare(r, f).getOccupant().getColor().equalsIgnoreCase("Black")) {
       				if(counter == 0) {
       					return output;
       				} else {
       					counter--;
